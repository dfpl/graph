...
public class MyPersistentGraph implements Graph {
	...
	public Edge addEdge(Vertex outVertex, Vertex inVertex, String label)
		...
		if (this.md.getCollection("edge")
				.find(Filters.eq("_id", inVertex.getId() + "|" + label + "|" + outVertex.getId())).first() == null) {
			Document doc = new Document();
			Document propertyDoc = new Document();
			...
			ArrayList<String> edgeIDsForInVertex = (ArrayList<String>) this.md.getCollection("vertex")
					.find(Filters.eq("_id", inVertex.getId())).first().get("outEdgeIDs");
			edgeIDsForInVertex.add(inVertex.getId() + "|" + label + "|" + outVertex.getId());

			this.md.getCollection("vertex").findOneAndUpdate(Filters.eq("_id", inVertex.getId()),
					Updates.set("outEdgeIDs", edgeIDsForInVertex));

			@SuppressWarnings("unchecked")
			ArrayList<String> edgeIDsForOutVertex = (ArrayList<String>) this.md.getCollection("vertex")
					.find(Filters.eq("_id", outVertex.getId())).first().get("inEdgeIDs");
			edgeIDsForOutVertex.add(inVertex.getId() + "|" + label + "|" + outVertex.getId());

			this.md.getCollection("vertex").findOneAndUpdate(Filters.eq("_id", outVertex.getId()),
					Updates.set("inEdgeIDs", edgeIDsForOutVertex));

			MyPersistentEdge edge = new MyPersistentEdge(inVertex.getId() + "|" + label + "|" + outVertex.getId(),
					label, md);
	...
	public Edge getEdge(String id) {
		...
		if (this.md.getCollection("edge").find(Filters.eq("_id", id)).first() != null)
			return new MyPersistentEdge(id,
					this.md.getCollection("edge").find(Filters.eq("_id", id)).first().getString("label"), md);
		else
			throw new IllegalArgumentException("no exist edge");
