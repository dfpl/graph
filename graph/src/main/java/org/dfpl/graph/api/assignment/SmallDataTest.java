...
public class SmallDataTest {
	...
	public static void main(String[] args) throws Exception {
		...
		// 참고: 평가를 위한 데이터셋은 보다 작은 데이터셋을 활용 (예: CollegeMsg.txt,
		...
		// Graph g = new YGraph(dbID, dbPW, dbName);
		...
		for (int i = 0; i < 10; i++) {
			...
			min7C = g.getVertex(maxOutDegreeID).getVertices(Direction.OUT).stream()
					.flatMap(v -> v.getVertices(Direction.OUT).stream()).toList().size();
		...
		for (int i = 0; i < 10; i++) {
			long pre = System.nanoTime();
			min8C = g.getVertex(maxInDegreeID).getVertices(Direction.IN).stream()
					.flatMap(v -> v.getVertices(Direction.IN).stream()).toList().size();
			long elapsedTime = System.nanoTime() - pre;
			if (elapsedTime < min8) {
		...
		for (int i = 0; i < 5; i++) {
			long pre = System.nanoTime();
			min11C = g.getVertex(maxOutDegreeID).getVertices(Direction.OUT).stream()
		...
		for (int i = 0; i < 5; i++) {
			long pre = System.nanoTime();
			min12C = g.getVertex(maxInDegreeID).getVertices(Direction.IN).stream()
